...
try:
    ...
    print("torch_npu is not available, please install it if you want to use npu")
    use_npu = False
...
def main():
    ...
    if args.gpu != -1:
        ...
        args.device = f"npu:{args.gpu}" if use_npu else f"cuda"
    ...
    with torch.cuda.amp.autocast(enabled=True,
        ...
        with torch.no_grad():
            ...
            for batch_idx, batch in enumerate(test_data_loader):
                ...
                res_text = model.generate4emotion_only(batch, device)
                for mode in modes:
